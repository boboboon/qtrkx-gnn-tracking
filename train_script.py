...
###############################################################################
def batch_train_step(n_step,config,train_data,train_list,model,loss_fn,opt):
...
def main(config_input,RID_input):


   
    # Read config file
    config = load_config(config_input,RID_input)
    ...
    for epoch in range(epoch_start, config['n_epoch']):
        ...
        for n_step in range(config['n_train']//config['batch_size']):
            ...
            # iterate a step
            loss_eval, grads = batch_train_step(n_step,config,train_data,train_list,model,loss_fn,opt)
